...
void
pickSpatialNeighbors(const Scene &scene, pcg32_state &rng, int x, int y, int R, int N, ImageReservoir &imgReservoir,
                     std::vector<Reservoir> &reservoirs,std::vector<Vector2i> &neighbors) {
    ...
    for (int i = 0; i < N; i++) {
        ...
        neighbors.push_back(Vector2i(x1, y1));
    }
...
//                               ImageRay& imgRay,ImagePathVertex & imgPathVertex) {
//    for (Reservoir &r: reservoirs) {
//        if (r.W != 0) {
//            s.update(r.sample, luminance(target_function(scene, vertex, ray, r.sample)) * r.W * r.M);
//
...
//        s.M += r.M;
//        if (s.sample.light_id>=0){
//            Vector2i pos = neighbors[i];
//            i++;
//            std::optional<PathVertex> vertex_ = imgPathVertex(pos.x,pos.y);
//            if (!vertex_) {
//                continue;
//            }
//            PathVertex neighbor_vertex = *vertex_;
//            if (luminance(target_function(scene, neighbor_vertex, imgRay(pos.x,pos.y) , s.sample))>0)
//                z += r.M;
//        }
//
//    }
...
//new version of restir path tracing
void combineReservoirsUnbiased(const Scene &scene,
                               PathVertex &vertex,
                               Ray &ray, std::vector<Reservoir> &reservoirs, Reservoir &s,
                               std::vector<Vector2i> & neighbors,
    ...
    for (Reservoir &r: reservoirs) {
        ...
        if (s.sample.light_id>=0){
            ...
            PathVertex neighbor_vertex = *vertex_;
            z += r.M*luminance(target_function(scene, neighbor_vertex, imgRay(pos.x,pos.y) , s.sample));
        }
...
// main function for restir path tracing
// restir + bsdf sampling
Spectrum restir_path_tracing_1(const Scene &scene,
    ...
    for (int num_vertices = 3; max_depth == -1 || num_vertices <= max_depth + 1; num_vertices++) {
        ...
        if (num_vertices == 3) {
            ...
            //spatial reuse
            //R: radius, N: number of samples(选多少像素) 从一个半径为R的圆内均匀采样N个点
            //sm in .xml file:
            std::vector<Vector2i> neighbors;
            neighbors.push_back(Vector2i(x,y));
            pickSpatialNeighbors(scene, rng, x, y, 30, 3, imgReservoir, reservoirs,neighbors);
            ...
            Reservoir r = imgReservoir(x,y);

            //combineReservoirs(scene, vertex, ray, reservoirs, r);

            combineReservoirsUnbiased(scene, vertex, ray, reservoirs, r, neighbors,imgRay,imgPathVertex);
            ...
            if (light_id >= 0) {
                ...
                if (G > 0 && p1 > 0) {
                    ...
                    //print R.W and p1:
                    //std::cout << "R.W: " << r.W << " p1: " << 1/p1 << std::endl;
                }
            }
